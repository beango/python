...
class BookspiderPipeline(object):
    ...
    def process_item(self, item, spider):
        dirPath = r'../book' + \
            '/' + item['categoryName'] + '/' + item['bookName']
    ...
    def process_item1(self, item, spider):
        self.cur.execute(
            "SELECT id FROM books_tag WHERE tagname = ?", (item['categoryName'],))
        tagID = self.cur.fetchone()
        if not tagID:
            self.cur.execute(
                "INSERT INTO books_tag (tagname) VALUES (?)", (item['categoryName'],))
            self.con.commit()
            self.cur.execute(
                "SELECT id FROM books_tag WHERE tagname = ?", (item['categoryName'],))
            tagID = self.cur.fetchone()
        ...
        self.cur.execute(
            "SELECT id FROM books_book WHERE title = ?", (item['bookName'],))
