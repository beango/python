...
class Pinger():
    ...
    def ping(self):
        cmdStr =  ('ping -n 1 %s > nul'%self.host).replace('\n','')
    ...
    def print_graph(self):
        ...
        print("-" * 53)
        print(u"|{0:16}{1:37}{2:16}|".format("",self.cyan(u"服务器延迟监测脚本"),""))
        print("-" * 53)
        print u"| {0:37}| {1:37}|".format(self.blue(u"上午统计"),self.blue(u"下午统计"))
        print(u"{0:39}| {1:37}|".format(u"| 最低延迟: " + self.colored(  min(self.morning_non_zero) if len(self.morning_non_zero)!=0 else 0),"最低延迟: " + self.colored(min(self.afternoon_non_zero) if len(self.afternoon_non_zero) != 0 else 0)  )  )
        print(u"{0:39}| {1:37}|".format(u"| 最高延迟: " + self.colored(max(self.morning_non_zero) if len(self.morning_non_zero)!=0 else 0),"最高延迟: " + self.colored(max(self.afternoon_non_zero) if len(self.afternoon_non_zero) != 0 else 0)))
        print(u"{0:39}| {1:37}|".format(u"| 平均延迟: " + self.colored(morning_average),"平均延迟: " + self.colored(afternoon_average)))
        print(u"{0:38}| {1:36}|".format(u"| 丢包率  : "+ self.lost_percentage(self.morning_zero,self.morning_non_zero),"丢包率  : " + self.lost_percentage(self.afternoon_zero,self.afternoon_non_zero)))
        print(u"{0:39}| {1:37}|".format(u"| 超时次数: " + self.red(len(self.morning_zero)),"超时次数: " + self.red(len(self.afternoon_zero))))
        print(u"{0:30}| {1:28}|".format(u"| 测试次数: " + str(len(self.morning_non_zero) + len(self.morning_zero)),"测试次数: " + str(len(self.afternoon_non_zero) + len(self.afternoon_zero))))



        print("-" * 53)
        print u"| {0:37}| {1:37}|".format(self.blue("夜晚统计"), self.blue("半夜统计"))
        print(u"{0:39}| {1:37}|".format(
        ...
        print(u"{0:39}| {1:37}|".format("| 平均延迟: " + self.colored(night_average),
                                       "平均延迟: " + self.colored(midnight_average)))
        print(u"{0:38}| {1:36}|".format("| 丢包率  : "+ self.lost_percentage(self.night_zero,self.night_non_zero),"丢包率  : " + self.lost_percentage(self.midnight_zero,self.midnight_non_zero)))
        print(u"{0:39}| {1:37}|".format("| 超时次数: " + self.red(len(self.night_zero)),
                                       "超时次数: " + self.red(len(self.midnight_zero))))
        print(u"{0:30}| {1:28}|".format("| 测试次数: " + str(len(self.night_non_zero) + len(self.night_zero)),
                                       "测试次数: " + str(len(self.midnight_non_zero) + len(self.midnight_zero))))


        print("-" * 53)
        print u"| {0:63}|".format(self.blue("全局统计"))
        print(u"{0:41}{1:36}|".format(u"| 最低延迟: " + self.colored(min(total_non_zero) if len(total_non_zero) != 0 else 0),"丢包率    : " + self.lost_percentage(total_zero_list,total_non_zero)))
        print(u"{0:41}{1:38}|".format(u"| 最高延迟: " + self.colored(max(total_non_zero) if len(total_non_zero) != 0 else 0),"总超时次数: " + self.red(total_zero)))
        print(u"{0:41}{1:29}|".format(u"| 平均延迟: " + self.colored(total_average),"总测试次数: " + str(total_count)))


        print("-" * 53)
        print (u"|{0:13}{1:30}{2:22}{3:9}|".format("", self.purple("当前测试Ping值:"), self.colored(self.data[self.data.keys()[-1]]["latency"]),""))
        print("-" * 53)

        print(u"|{0:12}{1:4}{2:2} 小时{3:2} 分钟{4:2} 秒{5:13}|".format("",self.purple("已耗时:"),int(spend_time/3600)if spend_time/3600 >=1 else 0,(int(spend_time % 3600 /60))if spend_time % 3600 /60 >= 1 else 0,int(spend_time % 3600 %60),""))
        print("-" * 53)
...
if __name__ == "__main__" :
    ...
    print(u"当前服务器时间为: " + str(mytime[0]) + "年" + str(mytime[1]) + "月" + str(mytime[2]) +"日 "  + str(mytime[3]) + ":" + str(mytime[4]) + ":" + str(mytime[5]) )
    
    pinghost = "192.168.1.158"
    p = Pinger(pinghost)
    p.start()
