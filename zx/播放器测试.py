...
# 8080表示目的端口
dest_addr = ('192.168.1.33', 9002)  # 注意 是元组，ip是字符串，端口是数字
...
for num in range(100,10000):
	no = no+1
	send_data = U"D,1,%d,S%04d,W,内儿科门诊,3002,黄宇,郑林娟,E" % (random.randint(1, 20), no)
	#udp_socket.sendto(send_data.encode('gb2312'), dest_addr)
	...
	# send_data = U'{"command":"play","playstyle":"1",speed:"45","volumn":"100","content":"please"}'
	#send_data = U"D,0001,%d,%d,W,内儿科门诊,3002,黄宇,张*3,E"%(random.randint(1, 10), num)
	#send_data = U"D,1,1,%d,W,内儿科门诊,3002,黄宇,张*3,E"%num
	print(send_data)
	udp_socket.sendto(send_data.encode('gb2312'), dest_addr)
...
while 0:
    no = random.randint(1, 20)*10
    for num in range(1, 11):
        no = no+1
        send_data = U"D,1,%d,S%04d,W,内儿科门诊,3002,黄宇,郑林娟,E" % (num, no)
        udp_socket.sendto(send_data.encode('gb2312'), dest_addr)
