...
# 连接MySQL数据库
db = web.database(dbn='mysql',host=settings.MYSQL_HOST, db='forum', user=settings.MYSQL_USERNAME, pw=settings.MYSQL_PASSWORD)
...
class Post:
    ...
    def list(self, page):
        ...
        for p in posts:
            ...
            page_posts.append({'id': p.id, 'title': p.title, 'userid': p.user_id, 'username': p.username, 'comment_count': comment.count(), 'time': p.time, 'last_time': last_time})
